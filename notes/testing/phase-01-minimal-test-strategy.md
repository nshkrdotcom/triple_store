...
- Crash harness tests (opt-in): `mix test --include crash_harness`
- All tests: `mix test --include crash_harness && (cd native/rocksdb_nif && cargo test)`
...
- Crash harness tests are excluded by default. They use `mix run --no-compile`
  in a subprocess to safely test NIF lifetime scenarios that could crash the VM.
...
- Elixir tests: 664 (662 normal + 2 crash harness)
- Rust unit tests: 5
